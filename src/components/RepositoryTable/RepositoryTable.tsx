...
import { SortOptions, ColumnHeaders } from "@/types/enums/enums";
...
export const RepositoryTable: React.FC<RepositoryTableProps> = ({
  ...
  const TableRows = repositories?.map((repo) => (
    <Table.Tr key={repo.name} c='cyan'>
      <Table.Td fw="bold">{repo.name}</Table.Td>
      <Table.Td>{truncateString(repo.description || "", 90)}</Table.Td>
      <Table.Td>{repo.language || "-"}</Table.Td>
      <Table.Td c ='violet'key="fork">{repo.forks_count}</Table.Td>
      <Table.Td c='blue'>{repo.stargazers_count}</Table.Td>
  ...
  const TableHeaders = (
    <Table.Thead>
      <Table.Tr style={{fontSize:'16px'}}>
        <Table.Th>{ColumnHeaders.Repository}</Table.Th>
